    ...
    <motion.article whileHover={{ y: -4 }} transition={{ duration: .2 }} className={cn("ticket-notch overflow-hidden border border-white/10 bg-[#111827]", featured && "lg:grid lg:grid-cols-[1.35fr_.65fr]")}>
      <Link href={`/events/${event.id}`} className={cn("event-art block min-h-48 p-5", `tone-${event.tone}`, featured && "lg:min-h-[380px] lg:p-8")}>
        ...
        <div className="relative z-10 flex h-full flex-col justify-between gap-14">
          ...
          <div>
            <div className="mb-3 flex flex-wrap gap-2">{event.tags.map(tag => <span key={tag} className="text-xs text-white/65">#{tag.toLowerCase().replace(" ", "-")}</span>)}</div>
      ...
      <div className={cn("relative flex flex-col justify-between border-t border-dashed border-white/15 p-5 lg:border-l lg:border-t-0", featured && "lg:p-7")}>
        <div>
          <div className="flex items-start justify-between gap-4">
            ...
            <span className="flex items-center gap-1 text-sm"><Star size={14} fill="#f59e0b" color="#f59e0b" /> {event.rating}</span>
          </div>
        ...
        <div className="mt-8 border-t border-white/8 pt-5">
          <div className="mb-4 flex items-end justify-between">
            <div><div className="eyebrow !text-slate-600">ENTRY</div><div className="mt-1 font-semibold">{formatEth(event.price)}</div></div>
            <div className="text-right"><div className="eyebrow !text-slate-600">CLAIMED</div><div className="mono mt-1 text-sm">{event.registered} / {event.capacity}</div></div>
          </div>
          <Link href={`/events/${event.id}`} className="flex items-center justify-between text-sm font-semibold text-[#60a5fa]">View admission <ArrowUpRight size={16} /></Link>
          <div className="mono mt-3 text-[10px] text-slate-600">HOST {shortAddress(event.organizer)}</div>
        </div>
      </div>
    </motion.article>
  );
